...
export interface IStorage {
  ...
  getGameStateByShareCode(shareCode: string): Promise<GameState | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getGroupByShareCode(shareCode: string): Promise<Group | undefined> {
    const [group] = await db.select().from(groups).where(eq(groups.shareCode, shareCode));
    return group;
  }
  ...
  async createGroup(insertGroup: InsertGroup): Promise<Group> {
    const [group] = await db.insert(groups).values({
      ...
      shareCode: insertGroup.shareCode || this.generateShareCode()
    }).returning();
    return group;
  }
  ...
  async getGameStateByShareCode(shareCode: string): Promise<GameState | undefined> {
    const [gameState] = await db.select().from(gameStates).where(eq(gameStates.shareCode, shareCode));
    return gameState;
  }
  ...
  async createGameState(insertGameState: InsertGameState): Promise<GameState> {
    const [gameState] = await db.insert(gameStates).values({
      ...
      shareCode: insertGameState.shareCode || this.generateShareCode()
    }).returning();
    return gameState;
  }
  ...
  async createPointsGame(insertPointsGame: InsertPointsGame): Promise<PointsGame> {
    const [pointsGame] = await db.insert(pointsGames).values(insertPointsGame).returning();
    return pointsGame;
  }
  ...
  // Utility method to generate share codes
